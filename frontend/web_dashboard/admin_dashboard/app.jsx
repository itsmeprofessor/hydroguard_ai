...
const App = () => {
  ...
  // ── Health WebSocket (/ws/health — public, no token) ─────────────────────
  const connectHealthWs = useCallback(() => {
    if (healthWsRef.current) { try { healthWsRef.current.close(); } catch {} }
    healthWsRef.current = API.connectWs("health",
      (data) => { if (data && data.cities) setCityHealth(data.cities); },
      () => {}
    );
  }, []);
  ...
  }, [view, connectWs, connectHealthWs, loadCities]);
  ...
  const screenProps = {
    city, cities, alertFiring, liveEvents, user, onNav: navTo, cityHealth,
  };
