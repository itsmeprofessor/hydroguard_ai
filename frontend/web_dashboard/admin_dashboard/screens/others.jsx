...
const MonitoringScreen = ({ liveEvents }) => {
  ...
  useEffect(() => {
    ...
    const line = `[${ts}] ${e.city} · score=${e.anomaly_score?.toFixed(3)} risk=${e.risk_level} HRI=${e.hri_score ?? "—"} CB=${e.cloudburst_risk?.is_likely ? "YES" : "NO"}`;
    setTerminal(prev => {
  ...
  const runPredict = async () => {
    ...
    try {
      const payload = {
        ...
        pressure:    parseFloat(form.pressure)|| undefined,
        dew_point:   parseFloat(form.dew_point)|| undefined,
        cloud_cover: parseFloat(form.cloud_cover)|| undefined,
      };
      const res = await API.predict(payload);
      setResult(res);
      toast(`Prediction complete: ${res.risk_level}`, res.risk_level === "CRITICAL" ? "crit" : res.risk_level === "HIGH" ? "warn" : "ok");
    } catch (e) {
      setError(e.message);
    ...
    <div className="screen">
      ...
      <div className="predict-grid">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {result ? (
            <>
              <Card label="Prediction Result" title={result.city}>
                <div className={`result-panel ${riskKind(result.risk_level)}`}>
                  <div className="mono-sm dim" style={{ letterSpacing: "0.1em" }}>ANOMALY SCORE</div>
                  <div className="result-value" style={{ color: riskColor(result.risk_level) }}>
                    {result.anomaly_score?.toFixed(3)}
                  </div>
                  <div className="row mt-8" style={{ gap: 10 }}>
                    <Status kind={riskKind(result.risk_level)}>{result.risk_level}</Status>
                    {result.hri_label && <span className="mono-sm muted">{result.hri_label}</span>}
                  </div>
                </div>
                <hr className="hr"/>
                <div className="mono-sm" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div className="row between"><span className="muted">Threshold</span><span className="mono">{result.threshold?.toFixed(4)}</span></div>
                  <div className="row between"><span className="muted">Is anomaly</span><Status kind={result.is_anomaly ? "crit" : "ok"}>{result.is_anomaly ? "YES" : "NO"}</Status></div>
                  <div className="row between"><span className="muted">HRI score</span><span className="mono">{result.hri_score ?? "—"}</span></div>
                  <div className="row between"><span className="muted">Consensus score</span><span className="mono">{result.consensus_score?.toFixed(4) ?? "—"}</span></div>
                  <div className="row between">
                ...
                {result.remarks && (
                  <div className="mt-12" style={{ padding: "8px 10px", background: "var(--bg-1)", borderRadius: 6, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
                    {result.remarks}
                  </div>
                )}
              </Card>
              ...
              {result.feature_contributions && (
                <Card label="Feature Contributions" title="Top drivers">
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {Object.entries(result.feature_contributions).slice(0, 8).map(([k, v]) => (
                      <div key={k}>
                        <div className="row between mono-sm mb-4">
                          <span style={{ color: "var(--text)" }}>{k}</span>
                          <span className="muted">{typeof v === "number" ? v.toFixed(4) : v}</span>
                        </div>
                        <div className="bar">
                          <span style={{ width: `${Math.min(Math.abs(parseFloat(v) || 0) * 200, 100)}%`, background: parseFloat(v) > 0 ? "var(--crit)" : "var(--ok)" }}/>
                        </div>
                      </div>
                    ))}
                  </div>
...
const SettingsScreen = ({ user }) => {
  ...
  const triggerRetrain = async () => {
    ...
    setTraining(true); setTrainMsg(""); setError("");
    try {
      const res = await API.triggerTraining({ use_lstm: useLstm, epochs: parseInt(epochs) || 100 });
      setTrainMsg(res.message || "Training started.");
      toast("Training started successfully.", "ok");
      // Refresh model info
      setTimeout(() => { API.modelInfo().then(d => setModelInfo(d)).catch(() => {}); }, 2000);
    } catch (e) {
      setError(e.message);
  ...
  return (
    <div className="screen">
      ...
      <div className="grid g-2 mb-16">
        <Card label="API Endpoints" title="Backend connections">
          <SettingsRow label="Prediction API"      val={`${API.BASE}/predict`}    status="ok"/>
          <SettingsRow label="Anomalies API"       val={`${API.BASE}/anomalies`}  status="ok"/>
          <SettingsRow label="Risk Map API"        val={`${API.BASE}/risk-map`}   status="ok"/>
      ...
      <div className="grid g-2">
        ...
        <Card label="Model · AE+LSTM" title="Training & deployment">
          {loading ? <LoadingState/> : (
            <>
              <div className="row between mono-sm" style={{ padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
              ...
              <div className="row between mono-sm" style={{ padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                ...
                <span className="mono">{modelInfo?.model_type || "—"}</span>
              </div>
              <div className="row between mono-sm" style={{ padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                <span className="muted">Input dim</span>
                <span className="mono">{modelInfo?.input_dim ?? "—"}</span>
              </div>
              <div className="row between mono-sm" style={{ padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                <span className="muted">Threshold</span>
                <span className="mono">{modelInfo?.threshold?.toFixed(4) ?? "—"}</span>
              </div>
              <div className="row between mono-sm" style={{ padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                <span className="muted">Features</span>
                <span className="mono">{modelInfo?.features?.length ?? "—"} columns</span>
              </div>
              ...
              {isAdmin && (
                <>
                  ...
                  <div className="row between mono-sm mb-12">
                    <span className="muted">Use LSTM</span>
                    <Toggle on={useLstm} onChange={setUseLstm}/>
                  </div>
                  <button className="btn btn-primary"
                    style={{ width: "100%", justifyContent: "center" }}
                    onClick={triggerRetrain} disabled={training}>
                    {training ? <><Spinner size={13}/>Training…</> : <><Icon name="brain"/>Trigger retrain</>}
                  </button>
                </>
              )}
              {!isAdmin && (
                <div className="mt-12 mono-sm muted" style={{ fontSize: 11, textAlign: "center" }}>
                  Retrain requires ADMIN role · your role: {user?.role || "—"}
                </div>
              )}
            </>
          )}
